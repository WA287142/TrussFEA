...
def readInputs(fname, nodes):
    ...
    for i in range(numNodes): # reads the lines with node #s on it and puts them into a node and xcoords array\
        line = f.readline().split()
    ...
    # print("Number of Constraints: ", numCons, " \nList of constrainedDOFs are: ", NOTdofs)
...
def getElementDOF(element):
    ...
    elemDOFs = []
    nodeValues = elemDict[element] # stores the node values in nodeValues
    
    
    temp = nodeDOFs[int(nodeValues[0])] # Stores the dofs of the first node in temp
    elemDOFs.append(int(temp[0]))
    elemDOFs.append(int(temp[1]))
    
    
    temp = nodeDOFs[int(nodeValues[1])] # Stores the dofs of the second node in temp
    elemDOFs.append(int(temp[0]))
    elemDOFs.append(int(temp[1]))
    # print("nodes for element", element, ":", elemDict[element])
    ...
    for i in range(numElems):
        ...
        for row in range(len(elemDOFs)):
            ...
            for col in range(len(elemDOFs)):
                ...
                globalstiff[globalRow, globalCol] = globalstiff[globalRow, globalCol] + elemK[row,col]
...
def imposeConstraints():
    ...
    for i in NOTdofs:
        ...
        # globalstiff[int(i)-1, int(i)-1] = globalstiff[int(i)-1,int(i)-1] + (1 * 10**30)
...
def calulateLocalForce(element):
    ...
    elemNodes = elemDict[element]
    node1 = nodes[int(elemNodes[0])] # node1 is a list of x y coords, elemNodes[0] is the first node
    node2 = nodes[int(elemNodes[1])]
    node1x = node1[0]
    node1y = node1[1]
    node2x = node2[0]
    ...
    l = np.sqrt((node2y - node1y)**2 + (node2x - node1x)**2)
    theta = np.arcsin((node2y - node1y)/l)                         # theta of element
    ...
    # print("T rotation: \n", tRot)
    tDim = np.zeros([2, 4])
    tDim[0,0] = 1                    #T dimensional, [[1 0 0 0], [0 0 1 0]]
    tDim[1,2] = 1
    # print("TDim = ", TDim)
    trussT = tDim @ tRot               # tRot is a 4x4, TDim is 2x4 = 2x4

    q1 = nodaldisplacements[int(elemNodes[0])]  # elemNodes[0] is the 1st node, disp1 is the deflection at that node
    q_x1 = q1[0]
    q_y1 = q1[1]
    q2 = nodaldisplacements[int(elemNodes[1])]  # elemNodes[1] is the 2nd node, disp1 is the deflection at that node
    q_x2 = q2[0]
    q_y2 = q2[1]  

    qTruss = np.zeros([4,1])        # global hori and vert deflections of element e , 4x1 matrix
    qTruss[0,0] = q_x1
    qTruss[1,0] = q_y1
    qTruss[2,0] = q_x2
    qTruss[3,0] = q_y2
    ...
    q_e = trussT @ qTruss       # trussT 2x4 and qTruss a 4x1 makes a 2x1
    # print("q_e = \n", q_e)
    # Q_e should be 1x4 or 4x1, K_e @ q_e -> 1x4, q_e is 2x1 so K_e has to be a 4x2 but is a 4x4 right now
    ...
    print("Element stress at two ends for element ", element, " : ", ax_stress, ax_stress)
...
def show_truss(q_sol, elem_num, elems_info, nodes_info):
    ''' function entries:
        1. q_sol: LIST of solutions to global nodal displacements 
            - format: [qx_node1, qy_node1, qx_node2, qy_node2, ...,qx_nodeN, qy_nodeN], where;  
                - qx_nodeN represents displacement of node N in x-direction
        ...
        3. elems_info: element connectivity information
            - format: [[elem1, elem1_node1, elem1_node2], [elem2, elem2_node1, elem2_node2], ...,[elemN, elemN_node1, elemN_node2]] where; 
            - elemN represents the element number N (an integer)
        ...
        4. nodes_info: node global coordinates
            - format: [[node1, x1_coord, y1_coord], [node2, x2_coord, y2_coord], ...,[nodeN, xN_coord, yN_coord]] where; 
                - nodeN represents node number N (an integer)
